...
rule VGsim:
    ...
    threads: 4,
    shell:
        "python3 ./bin/VGsim_cmd.py -rt mutation_data/twosite.rt -it {config[vg_iterations]} -s {config[vg_samples]} -pm {config[population_params]}.pp {config[population_params]}.mg --createNewick {config[output_directory]}/newick_output --writeMigrations {config[output_directory]}/migrations"
